...
{:>10} t: `{:?}`: {}"#),
    ...
    /*
    ($left:expr, $right:expr, abs_diff_all <= $max_abs_diff_all:expr, relative_all <= $max_relative_all:expr $(,)?) => {{
        match (&$left, &$right, &$max_abs_diff_all, &$max_relative_all) {
            (left_val, right_val, max_abs_diff_all_val, max_relative_all_val) => {
                if !$crate::relative_eq!(*left_val, *right_val, abs_diff_all <= *max_abs_diff_all_val, relative_all <= *max_relative_all_val) {
                    // The reborrows below are intentional. Without them, the stack slot for the
                    // borrow is initialized even before the values are compared, leading to a
                    // noticeable slow down. See the documentation for `core::approx_eq`.
                    panic!(concat!(
"assertion failed: `assert_relative_eq!(left, right, abs_diff_all <= t, max_relative_all <= t)`", r#"
        left: `{:?}`,
       right: `{:?}`,
    abs_diff: `{:?}`,
{:>10} t: `{:?}`,
{:>10} t: `{:?}`"#),
                        ...
                        $crate::AssertAbsDiffEqAll::debug_all_relative_tolerance(&*left_val, &*right_val, &*tol_val),
                    )
                }
            }
        }
    }};
    ($left:expr, $right:expr, relative_all <= $tol:expr, $($arg:tt)+) => {{
        match (&$left, &$right, &$tol) {
            (left_val, right_val, tol_val) => {
                if !$crate::relative_eq!(left_val, right_val, relative_all <= tol_val) {
                    // The reborrows below are intentional. Without them, the stack slot for the
                    // borrow is initialized even before the values are compared, leading to a
                    // noticeable slow down. See the documentation for `core::approx_eq`.
                    panic!(
                        concat!("assertion failed: assert_relative_eq!(left, right, abs_diff_all <= t)\nleft: `{:?}`,\nright: `{:?}`,\nabs_diff: `{:?}`,\nt: `{:?}`\n{}"),
                        &*left_val,
                        &*right_val,
                        $crate::AssertAbsDiffEqAll::debug_all_abs_diff(&*left_val, &*right_val),
                        $crate::AssertAbsDiffEqAll::debug_all_relative_tolerance(&*left_val, &*right_val, &*tol_val),
...
{:>10} t: `{:?}`: {}"#),
    ...
    ($left:expr, $right:expr, abs_diff_all <= $max_abs_diff_all:expr, relative_all <= $max_relative_all:expr $(,)?) => {{
        match (&$left, &$right, &$max_abs_diff_all, &$max_relative_all) {
            (left_val, right_val, max_abs_diff_all_val, max_relative_all_val) => {
                if !$crate::relative_ne!(*left_val, *right_val, abs_diff_all <= *max_abs_diff_all_val, relative_all <= *max_relative_all_val) {
                    // The reborrows below are intentional. Without them, the stack slot for the
                    // borrow is initialized even before the values are compared, leading to a
                    // noticeable slow down. See the documentation for `core::approx_eq`.
                    panic!(concat!(
"assertion failed: `assert_relative_ne!(left, right, abs_diff_all <= t, max_relative_all <= t)`", r#"
        left: `{:?}`,
       right: `{:?}`,
    abs_diff: `{:?}`,
{:>10} t: `{:?}`,
